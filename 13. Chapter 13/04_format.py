# name = "Chaitannyaa"
# channel = "CodeWithChaitannyaa"
# type = "Coding"
# # a = f"This is {name}"
# # a = "This is {}".format(name)
# # a = "This is {} and his channel is {}".format(name, channel)
# # a = "This is {0} and his {2} channel is {1}".format(name, channel, type)
# # a = "This is {} and his {} channel is {}".format(name, channel, type)
# a = "This is {} and his {} channel is for {}".format(name, channel, type)
# b = "This is {0} and his {2} channel is {1}".format(name, channel, type)
# print(a)
# print(b)

a = "{} is nice boy - {}".format("Chaitannyaa", "GOD")
print(a)